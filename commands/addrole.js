...
module.exports.run = async (client, message, args) => {
  if (!message.member.hasPermission('MANAGE_ROLES'))
    return errors.noPermissions(message, 'MANAGE_ROLES');
  ...
  if (user.roles.has(guildRole.id))
    return message.reply('That role does exist!');
  ...
  console.log(
    `[${message.guild}] ${message.author.username} has assigned the role ${guildRole.name} to ${user.user.username}.`
  );
  return;
};
